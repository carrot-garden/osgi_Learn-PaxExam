 ...
 * 
 * You can recognize the "JUnit Driver" approach by the @RunWith() annotation at
 * class level.
 * 
 * This overloads JUnit4s default runner so that Pax Exam is in full control of:
 * - the test roaster - the test invocation
 * 
 * So what's the test roaster ? You know methods annotated with @Test
 * annotations from JUnit4 API, right ?
 * 
 * This is the standard roaster. Those will appear in your Test Runner when
 * launching this class in your IDE using "Run with JUnit..".
 * 
 ...
 * 
 * Thats what the @RunWith(JUnit4TestRunner.class) does. Pax Exam rearranges the
 * JUnit4 Roaster and gives you a single entry for each physical test. In this
 * lesson we are using the PaxRunnerTestContainer implementation (see the
 * pom.xml), and additional put two OSGi Frameworks to it: Felix and Equinox.
 * 
 * You will see each of the @Test methods below twice. Once for each framework.
 * 
 * The @Configuration is a declarative way of what you did manually in the
...
public class LessonTest {
	...
	public static void main(final String[] args) throws TimeoutException,
			IOException {
		createContainer(
				createTestSystem(combine(new LessonTest().config(),
						profile("gogo")))).start();
	}
	 ...
	 */
	@Test
	public void withBC(final BundleContext ctx) {
	...
	@Test
	public TestAddress prebuilt(final TestProbeBuilder builder) {
